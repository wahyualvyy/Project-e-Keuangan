        ...
        <div class="card-body">
            <div class="d-md-flex align-items-center">
                ...
                <div class="ms-auto mt-3 mt-md-0 d-flex gap-2">
                    ...
                    <select class="form-select theme-select border-0" name="sort" id="filterSort" style="width: auto;">
                        ...
                        <option value="belum_lunas" <?= ($sort === 'belum_lunas') ? 'selected' : ''; ?>>Belum Lunas</option>
                    </select>
                </div>
            </div>

            <!-- Bulk Action Form -->
            <?php if (!empty($pembayaran_semester)): ?>
            <form action="<?= base_url('kas-masuk/semester/bulk-action'); ?>" method="POST" id="bulkActionForm">
                <?= csrf_field(); ?>
                
                <div class="d-flex align-items-center gap-2 mt-3">
                    <select class="form-select" name="aksi_massal" id="aksi-massal-select" style="width: auto;">
                        <option value="">Pilih Aksi</option>
                        <option value="bayar">Bayar Terpilih</option>
                        <option value="export_excel">Export Excel</option>
                        <option value="hapus">Hapus Terpilih</option>
                    </select>
                    <button type="button" class="btn btn-primary btn-bulk-delete">
                    ...
                    <table class="table mb-0 text-nowrap varient-table align-middle fs-3">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="px-0 text-muted">Semester</th>
                                <th scope="col" class="px-0 text-muted">Tahun Ajaran</th>
                                <th scope="col" class="px-0 text-muted">Biaya Semester</th>
                                <th scope="col" class="px-0 text-muted">Status</th>
                                <th scope="col" class="px-0 text-muted text-center">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php $i = 1; foreach ($pembayaran_semester as $data): ?>
                                <tr>
                                    <td class="px-0">
                                        <input type="checkbox" name="id_pembayaran_semester[]" 
                                               value="<?= $data['id_pembayaran_semester']; ?>" 
                                    ...
                                    <td class="px-0">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="ms-3">
                                                ...
                                                <small class="text-muted"><?= $data['nama_kelas'] ?? '-'; ?></small>
                                            </div>
                                        </div>
                                    </td>
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item" 
                                                           href="<?= base_url('kas-masuk/semester/bayar/' . $data['id_pembayaran_semester']); ?>">
                                                            <i class="ti ti-cash-banknote fs-6"></i>
                                                            <span class="fs-3">Bayar</span>
                                                        </a>
                                                    </li>
                                                <?php endif; ?>
                                                <li>
                                                    <a class="dropdown-item text-danger btn-delete-single" 
                                                       href="<?= base_url('kas-masuk/semester-delete/' . $data['id_pembayaran_semester']); ?>">
                                                        <i class="ti ti-trash fs-6"></i>
                                                        <span class="fs-3">Hapus</span>
                                                    </a>
                                                </li>
                                            </ul>
                                        </div>
            ...
            <?php else: ?>
                <div class="alert alert-info mt-4" role="alert">
                    ...
                    Tidak ada data pembayaran semester yang tersedia.
                </div>
            <?php endif; ?>
        </div>
    </div>
